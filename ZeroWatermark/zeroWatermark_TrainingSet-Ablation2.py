...
class WatermarkGenerator_Ablation2:
    """零水印生成器 - 消融实验2（仅图级特征）"""
    
    def __init__(self, model_path=None):
        if model_path is None:
            model_path = os.path.join(os.path.dirname(__file__), '..', 'VGAT', 'models', 'gat_model_Ablation2_GraphOnly_best.pth')
    ...
    def load_model(self):
        """加载消融实验2模型"""
        try:
            print("加载消融实验2模型（仅图级特征）...")
            if os.path.exists(self.model_path):
                import importlib.util
                
                base_dir = os.path.normpath(os.path.join(os.path.dirname(__file__), '..'))
                ablation2_path = os.path.join(base_dir, 'VGAT', 'Ablation2_GraphOnly.py')
                ...
                sys.modules['Ablation2_GraphOnly'] = ablation2_module
                spec.loader.exec_module(ablation2_module)
                
                GraphOnlyModel = ablation2_module.GraphOnlyModel
                
                self.model = GraphOnlyModel(input_dim=20, hidden_dim=256, output_dim=1024, num_heads=8, dropout=0.3)
                ...
                print("✅ 消融实验2模型加载完成")
                print(f"  图级特征维度: 8 (dims: [0,1,2,11,12,13,18,19])")
                print(f"  隐藏维度: 256")
                print(f"  输出维度: 1024")
            else:
...
def main():
    """主函数"""
    print("="*70)
    print("零水印生成 - 消融实验2（仅图级特征）")
    print("="*70)
    
    base_dir = os.path.dirname(os.path.abspath(__file__))
    graph_dir = os.path.join(base_dir, '..', 'convertToGraph', 'Graph', 'TrainingSet', 'Original')
    output_dir = os.path.join(base_dir, 'ZeroWatermark', 'TrainingSet-Ablation2')
    ...
    try:
        generator = WatermarkGenerator_Ablation2()
    except Exception as e:
        print(f"❌ 模型加载失败: {e}")
        return
