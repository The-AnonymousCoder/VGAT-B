...
class WatermarkGenerator_Ablation3:
    """零水印生成器 - 消融实验3（混合单流）"""
    
    def __init__(self, model_path=None):
        if model_path is None:
            model_path = os.path.join(os.path.dirname(__file__), '..', 'VGAT', 'models', 'gat_model_Ablation3_MixedSingle_best.pth')
    ...
    def load_model(self):
        """加载消融实验3模型"""
        try:
            print("加载消融实验3模型（混合单流）...")
            if os.path.exists(self.model_path):
                import importlib.util
                
                base_dir = os.path.normpath(os.path.join(os.path.dirname(__file__), '..'))
                ablation3_path = os.path.join(base_dir, 'VGAT', 'Ablation3_MixedSingleStream.py')
                
                spec = importlib.util.spec_from_file_location("Ablation3_MixedSingleStream", ablation3_path)
                ablation3_module = importlib.util.module_from_spec(spec)
                sys.modules['Ablation3_MixedSingleStream'] = ablation3_module
                spec.loader.exec_module(ablation3_module)
                
                MixedSingleStreamGATModel = ablation3_module.MixedSingleStreamGATModel
                
                self.model = MixedSingleStreamGATModel(input_dim=20, hidden_dim=256, output_dim=1024, num_heads=8, dropout=0.3)
...
def main():
    """主函数"""
    print("="*70)
    print("零水印生成 - 消融实验3（混合单流）")
    print("="*70)
    
    base_dir = os.path.dirname(os.path.abspath(__file__))
    graph_dir = os.path.join(base_dir, '..', 'convertToGraph', 'Graph', 'TrainingSet', 'Original')
    output_dir = os.path.join(base_dir, 'ZeroWatermark', 'TrainingSet-Ablation3')
    ...
    try:
        generator = WatermarkGenerator_Ablation3()
    except Exception as e:
        print(f"❌ 模型加载失败: {e}")
        return
