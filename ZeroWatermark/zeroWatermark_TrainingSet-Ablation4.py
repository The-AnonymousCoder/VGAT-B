...
class WatermarkGenerator_Ablation4:
    """零水印生成器 - 消融实验4（单注意力头）"""
    
    def __init__(self, model_path=None):
        if model_path is None:
            model_path = os.path.join(os.path.dirname(__file__), '..', 'VGAT', 'models', 'gat_model_Ablation4_SingleHead_best.pth')
    ...
    def load_model(self):
        """加载消融实验4模型"""
        try:
            print("加载消融实验4模型（单注意力头）...")
            if os.path.exists(self.model_path):
                import importlib.util
                
                base_dir = os.path.normpath(os.path.join(os.path.dirname(__file__), '..'))
                ablation4_path = os.path.join(base_dir, 'VGAT', 'Ablation4_SingleHead.py')
                ...
                sys.modules['Ablation4_SingleHead'] = ablation4_module
                spec.loader.exec_module(ablation4_module)
                
                SingleHeadGATModel = ablation4_module.SingleHeadGATModel
                
                self.model = SingleHeadGATModel(input_dim=20, hidden_dim=256, output_dim=1024, num_heads=1, dropout=0.3)
...
def main():
    """主函数"""
    print("="*70)
    print("零水印生成 - 消融实验4（单注意力头）")
    print("="*70)
    
    base_dir = os.path.dirname(os.path.abspath(__file__))
    graph_dir = os.path.join(base_dir, '..', 'convertToGraph', 'Graph', 'TrainingSet', 'Original')
    output_dir = os.path.join(base_dir, 'ZeroWatermark', 'TrainingSet-Ablation4')
    ...
    try:
        generator = WatermarkGenerator_Ablation4()
    except Exception as e:
        print(f"❌ 模型加载失败: {e}")
        return
