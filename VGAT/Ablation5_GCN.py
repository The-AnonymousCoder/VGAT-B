...
架构特点：
- 保持完整的双流架构
- 将GATv2Conv替换为GCNConv
- 用于验证动态注意力机制的贡献

对比完整模型：GCN使用固定权重的邻域聚合，GAT使用动态学习的注意力权重
"""
...
# 设置独立的日志记录器
def setup_ablation_logging(ablation_name="Ablation5_GCN"):
    """设置消融实验的独立日志"""
    ...
    logger.info(f"消融试验5：GCN模型训练")
    logger.info(f"日志文件: {log_file}")
    logger.info(f"最新日志(覆盖): {latest_file}")
    return logger

logger = setup_ablation_logging("Ablation5_GCN")
...
class GCNModel(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim=256, output_dim=1024, num_heads=8, dropout=0.3):
        ...
        logger.info(f"创建【消融5：GCN】模型:")
        logger.info(f"  ⭐ 使用GCN替代GAT（固定聚合权重 vs 动态注意力）")
        ...
        # ⭐ 关键差异：使用GCN层
        # GCN不支持多头，因此直接输出hidden_dim维度
        self.gcn1 = GCNConv(self.node_input_dim, hidden_dim)
        ...
        logger.info(f"  GCN聚合方式: 归一化的固定权重 (不可学习)")
        logger.info(f"  对比GAT: 动态学习的注意力权重")
        ...
        logger.info(f"  对比：GAT参数量更多（包含注意力权重）")
    ...
    def forward(self, x, edge_index, batch=None):
        ...
        # ⭐ 步骤2：GCN处理节点级特征（固定聚合权重）
        x1 = self.gcn1(node_features, edge_index)
        x1 = self.ln1(x1)
        x1 = F.elu(x1)
        x1 = self.dropout(x1)
...
def main():
    ...
    if len(original_graphs) == 0:
        logger.warning("没有找到原始图数据！")
        return
    
    if len(attacked_graphs) == 0:
        logger.warning("没有找到攻击图数据！")
        return
    ...
    if input_dim == 20:
        logger.info("原始维度 20")
        logger.info("  Hu + 5 + 5 + 5 + 5")
    elif input_dim == 19:
        logger.info("原始维度 19")
        logger.info("  Hu + 5 + 5 + 4")
    elif input_dim == 16:
        logger.info("原始维度 16")
        logger.info("  A + 5 + 5 + 6")
    elif input_dim == 13:
        ...
        logger.warning(f"未知维度: {input_dim}")
    ...
    # 获取批大小
    try:
        configured_bs = int(os.environ.get('VGAT_BATCH_SIZE', '6'))
    except Exception:
        configured_bs = 6
    
    logger.info(f"批大小: {configured_bs}")
    logger.info(f"批大小 {configured_bs} x 4 x 2 x 1 ( 2 x 3 x 4 x 5 )")
    logger.info(f"大于30000")
    
    # 批大小检查
    if configured_bs == 6:
        logger.info(f"   批大小 6")
        logger.info(f"   2 x 3 x 4 x 5")
    elif configured_bs == 8:
        logger.warning(f"   批大小 8")
        logger.info(f"   2 x 3 x 4 x 5")
    elif configured_bs > 8:
        logger.warning(f"   批大小 {configured_bs} ")
        logger.warning(f"   2 x 3 x 4 x 5")
    elif configured_bs == 4:
        logger.warning(f"   批大小 4")
        logger.info(f"   2 x 3 x 4 x 5")
    elif configured_bs <= 2:
        logger.warning(f"   批大小 ")
        logger.warning(f"   2 x 3 x 4 x 5")
    else:
        logger.info(f"   批大小 {configured_bs}")
        logger.info(f"   2 x 3 x 4 x 5")
    ...
    trainer = ImprovedContrastiveTrainer(
        ...
        checkpoint_name='ablation5_gcn_checkpoint.pth',  # 独立checkpoint，避免覆盖
        model_prefix='Ablation5_GCN'  # 独立模型文件名，避免覆盖
    )
    ...
    # 检查是否恢复训练
    script_dir = os.path.dirname(os.path.abspath(__file__))
    checkpoint_path = os.path.join(script_dir, 'checkpoints', 'ablation5_gcn_checkpoint.pth')
    
    resume_training = os.environ.get('VGAT_RESUME_TRAINING', 'auto').lower()
    resume_from = None
    
    if resume_training == 'auto':
        if os.path.exists(checkpoint_path):
            logger.info(f"找到checkpoint文件: {checkpoint_path}")
            user_choice = input("是否恢复训练?[y/N]: ").strip().lower()
    ...
    elif resume_training in ['true', '1']:
        if os.path.exists(checkpoint_path):
            resume_from = checkpoint_path
            logger.info(f"恢复训练: {checkpoint_path}")
        else:
            logger.warning(f"没有找到checkpoint文件")
    else:
        logger.info("不恢复训练")
    ...
    # 保存模型
    script_dir = os.path.dirname(os.path.abspath(__file__))
    final_model_path = os.path.join(script_dir, 'models', 'gat_model_Ablation5_GCN.pth')
    best_model_path = os.path.join(script_dir, 'models', 'gat_model_Ablation5_GCN_best.pth')
