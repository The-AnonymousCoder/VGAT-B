...
def run_evaluation():
	...
	if KEEP_EXISTING and any(DIR_RESULTS.glob('*.csv')):
		print(f'⚠️ KEEP_EXISTING=True，且 Fig11 结果已存在于 {DIR_RESULTS}，跳过')
		return
	missing = []
	...
	for file_idx, (vector_file, file_name) in enumerate(zip(USE_FILES, USE_NAMES)):
		...
		for k_idx, key in enumerate(order_keys):
			try:
				...
				rows.append({'file': file_name, 'key': key, 'nc': 0.0})  # ← 关键修复：失败时也记录NC=0.0
	...
	for key in order_keys:
		...
		for file_name in USE_NAMES:  # ← 修复：使用USE_NAMES（非点数据）而不是FILE_NAMES
