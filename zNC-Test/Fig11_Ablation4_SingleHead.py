#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Fig11_Ablation4_SingleHead：使用消融实验4（单注意力头）的模型测试组合攻击鲁棒性"""
import sys
...
DIR_ZEROWM = SCRIPT_DIR / 'vector-data-zerowatermark-ablation4'
DIR_RESULTS = SCRIPT_DIR / 'NC-Results' / 'Fig11_Ablation4_SingleHead'
...
def load_ablation4_model(device):
    ...
    if not ABLATION_MODEL_PATH.exists():
        raise FileNotFoundError(f"消融实验4模型文件不存在: {ABLATION_MODEL_PATH}")
    ...
    model.eval()
    print(f"✅ 已加载消融实验4模型: {ABLATION_MODEL_PATH.name}")
    return model, device
...
def step5_generate_zero_watermark():
    print('[Step5] 生成零水印 (消融实验4) ->', DIR_ZEROWM)
...
def step6_evaluate_nc():
    print('[Step6] 评估NC (消融实验4) ->', DIR_RESULTS)
    if not DIR_GRAPH_ATTACKED.exists() or not ABLATION_MODEL_PATH.exists(): return
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    try: model, device = load_ablation4_model(device)
    ...
    for k in order_keys:
        ...
        hierarchical_rows.append({'组合攻击': label, 'Ablation4_SingleHead': '', '类型': 'header'})
        for base in base_names:
            sub = df[(df['base'] == base) & (df['combo_key'] == k)]
            nc_value = sub['nc'].iloc[0] if len(sub) > 0 else 0
            hierarchical_rows.append({'组合攻击': f'  {base}', 'Ablation4_SingleHead': f'{nc_value:.6f}', '类型': 'data'})
        avg_nc = df[df['combo_key'] == k]['nc'].mean()
        hierarchical_rows.append({'组合攻击': '  Average', 'Ablation4_SingleHead': f'{avg_nc:.6f}', '类型': 'average'})
    hierarchical_df = pd.DataFrame(hierarchical_rows)
    csv_path = DIR_RESULTS / 'fig11_ablation4_nc.csv'
    hierarchical_df.to_csv(csv_path, index=False, encoding='utf-8-sig')
    xlsx_path = DIR_RESULTS / 'fig11_ablation4_nc.xlsx'
    try:
    ...
    plt.xlabel('组合攻击'); plt.ylabel('NC'); plt.title('Fig11_Ablation4：组合攻击 NC鲁棒性 (单注意力头)')
    plt.legend(loc='best', fontsize=8, ncol=2)
    fig_path = DIR_RESULTS / 'fig11_ablation4_nc.png'
    plt.tight_layout(); plt.savefig(fig_path, dpi=300, bbox_inches='tight'); plt.close()
    print('曲线图保存: ', fig_path.name)

if __name__ == '__main__':
    print('=== Fig11_Ablation4：组合攻击鲁棒性测试 (单注意力头) ===')
