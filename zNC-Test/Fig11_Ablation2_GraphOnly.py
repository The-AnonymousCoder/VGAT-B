#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Fig11_Ablation2_GraphOnly：使用消融实验2（仅图级特征）的模型测试组合攻击鲁棒性"""
...
# ⭐ 强制覆盖配置
DIR_ZEROWM = SCRIPT_DIR / 'vector-data-zerowatermark-ablation2'
DIR_RESULTS = SCRIPT_DIR / 'NC-Results' / 'Fig11_Ablation2_GraphOnly'
...
def step6_evaluate_nc():
    ...
    for base_dir_path in sorted(DIR_GRAPH_ATTACKED.iterdir()):
        ...
        for pkl in sorted(base_dir_path.glob('*_graph.pkl')):
            file_count += 1
            if file_count % 10 == 0 and check_memory(90):
                print(f'\n❗ 处理{file_count}个文件后内存不足！'); break
            try:
                with open(pkl, 'rb') as f:
                    graph = pickle.load(f)
                feat_mat = extract_features_from_graph(graph, model, device, copyright_img.shape)
    ...
    for k in order_keys:
        ...
        hierarchical_rows.append({'组合攻击': label, 'Ablation2_GraphOnly': '', '类型': 'header'})
        for base in base_names:
            sub = df[(df['base'] == base) & (df['combo_key'] == k)]
            nc_value = sub['nc'].iloc[0] if len(sub) > 0 else 0
            hierarchical_rows.append({'组合攻击': f'  {base}', 'Ablation2_GraphOnly': f'{nc_value:.6f}', '类型': 'data'})
        avg_nc = df[df['combo_key'] == k]['nc'].mean()
        hierarchical_rows.append({'组合攻击': '  Average', 'Ablation2_GraphOnly': f'{avg_nc:.6f}', '类型': 'average'})
    hierarchical_df = pd.DataFrame(hierarchical_rows)
    csv_path = DIR_RESULTS / 'fig11_ablation2_nc.csv'
    hierarchical_df.to_csv(csv_path, index=False, encoding='utf-8-sig')
    xlsx_path = DIR_RESULTS / 'fig11_ablation2_nc.xlsx'
    try:
    ...
    plt.title('Fig11_Ablation2：组合攻击 NC鲁棒性 (仅图级特征)')
    plt.legend(loc='best', fontsize=8, ncol=2)
    fig_path = DIR_RESULTS / 'fig11_ablation2_nc.png'
    plt.tight_layout(); plt.savefig(fig_path, dpi=300, bbox_inches='tight'); plt.close()
    print('曲线图保存: ', fig_path.name)


if __name__ == '__main__':
    print('=== Fig11_Ablation2：组合攻击鲁棒性测试 (仅图级特征) ===')
