...
流程与 Fig1 对齐（Step1~Step6），包含跳步逻辑；
        ...
        if existing['step3']:
            for attacked_dir in DIR_VECTOR_GEOJSON_ATTACKED.glob('*'):
                if attacked_dir.is_dir():
                    ...
                    if geojson_file.exists(): attacked_map[base]['compound_seq_all'] = geojson_file
        else:
