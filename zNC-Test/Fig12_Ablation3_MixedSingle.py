...
# -*- coding: utf-8 -*-
"""
Fig12_Ablation3_MixedSingle：使用消融实验1（混合特征单流）的模型测试复合攻击鲁棒性

与Fig12.py逻辑完全一致，仅模型路径和输出目录不同：
- 模型：VGAT/models/gat_model_Ablation3_MixedSingle_best.pth
- 零水印输出：vector-data-zerowatermark-ablation3
- NC结果输出：NC-Results/Fig12_Ablation3_MixedSingle
"""
...
ABLATION_MODEL_PATH = PROJECT_ROOT / 'VGAT' / 'models' / 'gat_model_Ablation3_MixedSingle_best.pth'
...
DIR_ZEROWM = SCRIPT_DIR / 'vector-data-zerowatermark-ablation3'  # ⭐ 独立零水印目录
DIR_RESULTS = SCRIPT_DIR / 'NC-Results' / 'Fig12_Ablation3_MixedSingle'  # ⭐ 独立结果目录


def load_ablation3_model(device):
    """加载消融实验3的模型"""
    if not ABLATION_MODEL_PATH.exists():
        raise FileNotFoundError(f"消融实验3模型文件不存在: {ABLATION_MODEL_PATH}")
    
    # 动态导入消融模型
    import importlib.util
    ablation3_path = PROJECT_ROOT / 'VGAT' / 'Ablation3_MixedSingleStream.py'
    spec = importlib.util.spec_from_file_location("Ablation3_MixedSingle", ablation3_path)
    ablation3_module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(ablation3_module)
    
    MixedSingleStreamGATModel = ablation3_module.MixedSingleStreamGATModel
    
    # 创建模型实例（参数与训练时一致）
    model = MixedSingleStreamGATModel(
        input_dim=20,  # 输入20维完整特征，模型内部自动选择18维
        hidden_dim=256,
        output_dim=1024,
        num_heads=8,
        dropout=0.3
    ...
    model.eval()
    print(f"✅ 已加载消融实验3模型: {ABLATION_MODEL_PATH.name}")
    return model, device
...
def step5_generate_zero_watermark():
    """⭐ 用消融实验3模型生成零水印"""
    print('[Step5] 生成零水印 (消融实验3) ->', DIR_ZEROWM)
    
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model, device = load_ablation3_model(device)
...
def step6_evaluate_nc():
    print('[Step6] 评估NC (消融实验3) ->', DIR_RESULTS)
    ...
    if not ABLATION_MODEL_PATH.exists():
        print('消融实验3模型文件不存在: ', ABLATION_MODEL_PATH)
        return
    ...
    extract_dir = SCRIPT_DIR / 'extract' / 'watermark' / 'Fig12_Ablation3_MixedSingle'
    ...
    # 层次表
    hierarchical_rows = []
    hierarchical_rows.append({'复合攻击(顺序)': '复合(Fig1→Fig10)', 'Ablation3_MixedSingle': '', '类型': 'header'})
    for _, row in df.iterrows():
        hierarchical_rows.append({'复合攻击(顺序)': f"  {row['base']}", 'Ablation3_MixedSingle': f"{row['nc']:.6f}", '类型': 'data'})
    hierarchical_rows.append({'复合攻击(顺序)': '  Average', 'Ablation3_MixedSingle': f"{df['nc'].mean():.6f}", '类型': 'average'})

    hierarchical_df = pd.DataFrame(hierarchical_rows)
    csv_path = DIR_RESULTS / 'fig12_ablation3_nc.csv'
    hierarchical_df.to_csv(csv_path, index=False, encoding='utf-8-sig')
    xlsx_path = DIR_RESULTS / 'fig12_ablation3_nc.xlsx'
    try:
    ...
    plt.bar(x, vals, alpha=0.8, label='Ablation3_MixedSingle')
    plt.axhline(df['nc'].mean(), color='k', linestyle='--', label='平均')
    plt.xticks(x, bases, rotation=20)
    plt.ylabel('NC')
    plt.title('Fig12_Ablation3：复合攻击 NC鲁棒性 (混合特征单流)')
    plt.legend()
    fig_path = DIR_RESULTS / 'fig12_ablation3_nc.png'
    plt.tight_layout()
...
def main():
    print('=== Fig12_Ablation3：复合攻击鲁棒性测试 (混合特征单流) ===')
