...
# -*- coding: utf-8 -*-
"""
Fig12_Ablation1_NodeOnly：使用消融实验1（仅节点级特征）的模型测试复合攻击鲁棒性

与Fig12.py逻辑完全一致，仅模型路径和输出目录不同：
- 模型：VGAT/models/gat_model_Ablation1_NodeOnly_best.pth
- 零水印输出：vector-data-zerowatermark-ablation1
- NC结果输出：NC-Results/Fig12_Ablation1_NodeOnly
"""
...
DIR_ZEROWM = SCRIPT_DIR / 'vector-data-zerowatermark-ablation1'  # ⭐ 独立零水印目录
DIR_RESULTS = SCRIPT_DIR / 'NC-Results' / 'Fig12_Ablation1_NodeOnly'  # ⭐ 独立结果目录
...
def step6_evaluate_nc():
    ...
    extract_dir = SCRIPT_DIR / 'extract' / 'watermark' / 'Fig12_Ablation1_NodeOnly'
    ...
    # 层次表
    hierarchical_rows = []
    hierarchical_rows.append({'复合攻击(顺序)': '复合(Fig1→Fig10)', 'Ablation1_NodeOnly': '', '类型': 'header'})
    for _, row in df.iterrows():
        hierarchical_rows.append({'复合攻击(顺序)': f"  {row['base']}", 'Ablation1_NodeOnly': f"{row['nc']:.6f}", '类型': 'data'})
    hierarchical_rows.append({'复合攻击(顺序)': '  Average', 'Ablation1_NodeOnly': f"{df['nc'].mean():.6f}", '类型': 'average'})

    hierarchical_df = pd.DataFrame(hierarchical_rows)
    csv_path = DIR_RESULTS / 'fig12_ablation1_nc.csv'
    hierarchical_df.to_csv(csv_path, index=False, encoding='utf-8-sig')
    xlsx_path = DIR_RESULTS / 'fig12_ablation1_nc.xlsx'
    try:
    ...
    plt.bar(x, vals, alpha=0.8, label='Ablation1_NodeOnly')
    plt.axhline(df['nc'].mean(), color='k', linestyle='--', label='平均')
    plt.xticks(x, bases, rotation=20)
    plt.ylabel('NC')
    plt.title('Fig12_Ablation1：复合攻击 NC鲁棒性 (仅节点级特征)')
    plt.legend()
    fig_path = DIR_RESULTS / 'fig12_ablation1_nc.png'
    plt.tight_layout()
...
def main():
    print('=== Fig12_Ablation1：复合攻击鲁棒性测试 (仅节点级特征) ===')
