...
# -*- coding: utf-8 -*-
"""
Fig12_Ablation4_SingleHead：使用消融实验1（单头注意力）的模型测试复合攻击鲁棒性

与Fig12.py逻辑完全一致，仅模型路径和输出目录不同：
- 模型：VGAT/models/gat_model_Ablation4_SingleHead_best.pth
- 零水印输出：vector-data-zerowatermark-ablation4
- NC结果输出：NC-Results/Fig12_Ablation4_SingleHead
"""
...
ABLATION_MODEL_PATH = PROJECT_ROOT / 'VGAT' / 'models' / 'gat_model_Ablation4_SingleHead_best.pth'
...
DIR_ZEROWM = SCRIPT_DIR / 'vector-data-zerowatermark-ablation4'  # ⭐ 独立零水印目录
DIR_RESULTS = SCRIPT_DIR / 'NC-Results' / 'Fig12_Ablation4_SingleHead'  # ⭐ 独立结果目录


def load_ablation4_model(device):
    """加载消融实验1的模型"""
    ...
    ablation4_path = PROJECT_ROOT / 'VGAT' / 'Ablation4_SingleHead.py'
    spec = importlib.util.spec_from_file_location("Ablation4_SingleHead", ablation4_path)
    ablation4_module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(ablation4_module)
    
    SingleHeadGATModel = ablation4_module.SingleHeadGATModel
    
    # 创建模型实例（参数与训练时一致）
    model = SingleHeadGATModel(
        input_dim=20,  # 完整特征维度，模型内部会分离为节点级(10)和图级(8)
        hidden_dim=256,
        output_dim=1024,
...
def step5_generate_zero_watermark():
    ...
    model, device = load_ablation4_model(device)
...
def step6_evaluate_nc():
    ...
    if not ABLATION_MODEL_PATH.exists():
        print('消融实验1模型文件不存在: ', ABLATION_MODEL_PATH)
        return
    ...
    extract_dir = SCRIPT_DIR / 'extract' / 'watermark' / 'Fig12_Ablation4_SingleHead'
    ...
    # 层次表
    hierarchical_rows = []
    hierarchical_rows.append({'复合攻击(顺序)': '复合(Fig1→Fig10)', 'Ablation4_SingleHead': '', '类型': 'header'})
    for _, row in df.iterrows():
        hierarchical_rows.append({'复合攻击(顺序)': f"  {row['base']}", 'Ablation4_SingleHead': f"{row['nc']:.6f}", '类型': 'data'})
    hierarchical_rows.append({'复合攻击(顺序)': '  Average', 'Ablation4_SingleHead': f"{df['nc'].mean():.6f}", '类型': 'average'})

    hierarchical_df = pd.DataFrame(hierarchical_rows)
    csv_path = DIR_RESULTS / 'fig12_ablation4_nc.csv'
    hierarchical_df.to_csv(csv_path, index=False, encoding='utf-8-sig')
    xlsx_path = DIR_RESULTS / 'fig12_ablation4_nc.xlsx'
    try:
    ...
    plt.bar(x, vals, alpha=0.8, label='Ablation4_SingleHead')
    plt.axhline(df['nc'].mean(), color='k', linestyle='--', label='平均')
    plt.xticks(x, bases, rotation=20)
    plt.ylabel('NC')
    plt.title('Fig12_Ablation4：复合攻击 NC鲁棒性 (单头注意力)')
    plt.legend()
    fig_path = DIR_RESULTS / 'fig12_ablation4_nc.png'
    plt.tight_layout()
...
def main():
    print('=== Fig12_Ablation4：复合攻击鲁棒性测试 (单头注意力) ===')
